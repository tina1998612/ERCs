...
interface IERC7007 is IERC165, IERC721 {
    ...
    /**
     * @dev Mint token at `tokenId` given `prompt`, `aigcData`, `uri` and `proofOrState`.
     * @dev `proofOrState` means that we input the zk proof when using zkML and the final inference state when using opML as the verification method.
     *
     * Requirements:
     ...
     *
     * Optional:
     * - `proofOrState` should not include `aigcData` to save gas.
     */
    function mint(
    ...
    /**
     * @dev Verify the `prompt`, `aigcData` and `proofOrState`.
     */
    function verify(
        bytes calldata prompt,
        bytes calldata aigcData,
