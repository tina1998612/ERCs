...
const uri =
  '"name": "test", "description": "test", "image": "test", "aigc_type": "test", "proof_type": "test"';
...
describe("ERC7007Zkml.sol", function () {
  ...
  describe("mint", function () {
    it("should mint a token", async function () {
      ...
      await erc7007.addAigcData(tokenId, prompt, aigcData, validProof);
      await erc7007.mint(owner.address, prompt, uri, validProof);
    ...
    it("should not mint a token with invalid proof", async function () {
      ...
      const [owner] = await ethers.getSigners();
      await expect(
        erc7007.mint(owner.address, prompt, uri, invalidProof)
      ).to.be.revertedWith("ERC7007: invalid proof");
    });
    ...
    it("should not mint a token with same data twice", async function () {
      ...
      await erc7007.mint(owner.address, prompt, uri, validProof);
      await expect(
        erc7007.mint(owner.address, prompt, uri, validProof)
  ...
  describe("metadata", function () {
    it("should return token metadata", async function () {
      ...
      await erc7007.addAigcData(tokenId, prompt, aigcData, validProof);
      await erc7007.mint(owner.address, prompt, uri, validProof);
      expect(await erc7007.tokenURI(tokenId)).to.equal(
        `{"name": "test", "description": "test", "image": "test", "aigc_type": "test", "proof_type": "test", "prompt": "test", "aigc_data": "test"}`
      );
    });
  });
});
...
describe("ERC7007Opml.sol", function () {
  ...
  async function deployERC7007OpmlFixture() {
  ...
  describe("mint", function () {
    it("should mint a token", async function () {
      const erc7007 = await deployERC7007OpmlFixture();
      const [owner] = await ethers.getSigners();
      await erc7007.addAigcData(tokenId, prompt, aigcData, validProof);
      await erc7007.mint(owner.address, prompt, uri, validProof);
      expect(await erc7007.balanceOf(owner.address)).to.equal(1);
    ...
    it("should verify a finalized request", async function () {
      ...
      await erc7007.addAigcData(tokenId, prompt, aigcData, validProof);
      await erc7007.mint(owner.address, prompt, uri, validProof);
      expect(await erc7007.verify(prompt, aigcData, validProof)).to.equal(true);
    });
